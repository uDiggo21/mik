...
import { Plus, Trash2, ShoppingCart, Search, XCircle } from 'lucide-react'
...
export default function Vendas() {
  ...
  function adicionarProdutoPorId(idProduto) {
    ...
    if (itemExistente) {
      const novaQuantidade = Number(itemExistente.quantidade || 0) + qtd
        ...
        itens.map((item) =>
          item.produto_id === produto.id
            ? {
                ...
                subtotal: novaQuantidade * Number(item.preco_unitario || 0)
              }
            : item
        )
      )
  ...
  function removerItem(idProduto) {
    setItens(itens.filter((item) => item.produto_id !== idProduto))
  }
  ...
  async function finalizarVenda({ formaPagamento, valorRecebido }) {
    if (itens.length === 0) {
    ...
    try {
      ...
      for (const item of itens) {
        const { error: erroItem } = await supabase.from('venda_itens').insert([
          {
        ...
        const novoEstoque =
          Number(produtoAtual?.estoque_atual || 0) - Number(item.quantidade || 0)
      ...
      if (formaPagamento === 'fiado') {
        const { error: erroFiado } = await supabase.from('fiado').insert([
          {
      ...
      if (formaPagamento === 'dinheiro') {
        const troco = Number(valorRecebido || 0) - Number(total || 0)
  ...
  async function cancelarVenda(venda) {
    ...
    await supabase.from('financeiro').delete().eq('venda_id', venda.id)
    await supabase.from('fiado').delete().eq('venda_id', venda.id)
    await supabase.from('vendas').update({ status: 'cancelada' }).eq('id', venda.id)

    await registrarAuditoria({
      tipo: 'cancelamento',
      modulo: 'vendas',
      descricao: `Venda cancelada no valor de ${formatarMoeda(venda.total)}`,
      valor: venda.total,
  ...
  const vendasFiltradas = vendas.filter((v) => {
    ...
    return (
      String(v.clientes?.nome || '').toLowerCase().includes(textoBusca) ||
      String(v.forma_pagamento || '').toLowerCase().includes(textoBusca) ||
      String(v.status || '').toLowerCase().includes(textoBusca)
    )
  })
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-gray-800">PDV / Vendas</h1>
          <p className="text-sm text-gray-500">Gestão de vendas e movimentações</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2 bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">Busca vendas</label>
          ...
          <div className="grid grid-cols-6 gap-3 mb-4">
            <div className="col-span-2">
              <label className="text-xs text-gray-500 mb-1 block">Código barras</label>
              ...
              >
                ...
                {produtos.map((produto) => (
                  <option key={produto.id} value={produto.id}>
                    {produto.nome} — {formatarMoeda(produto.preco_venda)} — Estoque:{' '}
                ...
                {itens.length === 0 ? (
                  ...
                  itens.map((item) => (
                    <tr key={item.produto_id} className="border-b border-gray-100">
                      ...
                      <td className="px-4 py-3">{formatarMoeda(item.preco_unitario)}</td>
        ...
        <div className="space-y-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            <h2 className="font-semibold text-gray-800 mb-4">Resumo venda</h2>
